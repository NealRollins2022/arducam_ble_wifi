...
LOG_MODULE_REGISTER(nord_camera, CONFIG_VIDEO_LOG_LEVEL);
...
static void on_stream_schedule_timer_func(struct k_timer *timer)
{
    ...
    k_work_submit_to_queue(&nord_ac_work_q, &drv_data->buf_work);
}
...
static const struct video_driver_api arducam_nord_driver_api = {
...
static int arducam_mega_init(const struct device *dev)
{
    ...
    k_work_queue_init(&nord_ac_work_q);
    k_work_queue_start(&nord_ac_work_q, nord_ac_stack_area, K_THREAD_STACK_SIZEOF(nord_ac_stack_area),
        AC_PRIORITY, NULL);
